// ==UserScript==
// @name         LIB - Document / Library Control
...
// @grant        none
// @tag          LIB
// @updateURL    https://raw.githubusercontent.com/Bristow-Scripts/bristow-scripts/main/LIB---Document-Library-Control.user.js
// @downloadURL  https://raw.githubusercontent.com/Bristow-Scripts/bristow-scripts/main/LIB---Document-Library-Control.user.js
// ==/UserScript==
...
(function () {
    ...
    // yyyy-MM-dd -> DD-MMM-YYYY (e.g. 2027-03-14 -> 14-Mar-2027)
    ...
    // Pull "Verification Cycle", the DETAILS block, and the VERIFICATION HISTORY
    // block out of the freeform Description textarea.
    function parseDescription(text) {
        const result = { verificationCycle: '', generalInfo: '', verificationHistory: '' };
        if (!text) return result;

        const vcMatch = text.match(/Verification Cycle:\s*(\d+)/i);
        if (vcMatch) result.verificationCycle = vcMatch[1];

        const detailsIdx = text.search(/DETAILS:/i);
        const vhIdx = text.search(/VERIFICATION HISTORY:/i);

        if (detailsIdx !== -1) {
            const end = vhIdx !== -1 ? vhIdx : text.length;
            result.generalInfo = text.slice(detailsIdx + 'DETAILS:'.length, end).trim();
        }
        if (vhIdx !== -1) {
            result.verificationHistory = text.slice(vhIdx + 'VERIFICATION HISTORY:'.length).trim();
        }
        return result;
    }
    ...
    function getTagValue(tags, tagTypeName) {
        ...
        const t = tags.find(x => x.TagTypeName === tagTypeName);
        return t ? (t.TagValue || '') : '';
    }

    // Strip the part number and manufacturer name out of an item's long
    // description so it prints the same way the paper sheet does
    // ("Flight Director Indicator 772-5005-004   Rockwell Collins" -> "Flight Director Indicator").
    ...
    async function fetchItems(documentationId) {
        const url = `./EditDocumentation?documentationId=${encodeURIComponent(documentationId)}&handler=Items`;
        try {
            ...
            console.error('Failed to fetch items for print sheet:', e);
            return [];
        }
    }
    ...
    function buildPrintHTML(fields, parsedDesc, itemRows) {
        const capYesNo = capabilityToYesNo(fields.capability);
        ...
        <table class="lcs-items">
            <thead>
                <tr><th>MAN. #</th><th>PART NUMBER</th><th>DESCRIPTION</th><th>CAP</th></tr>
            </thead>
    ...
    async function printLibraryControlSheet() {
        const documentationId = val(FIELD.id);
        const fields = {
        ...
        const parsedDesc = parseDescription(val(FIELD.description));

        const rawItems = documentationId ? await fetchItems(documentationId) : [];
        const itemRows = rawItems.map(item => {
            const partNumber = getTagValue(item.Tags, 'Part Number');
            return {
                partNumber,
    ...
    function addPrintButton() {
        ...
        const heading = Array.from(document.querySelectorAll('h2')).find(h => h.textContent.trim() === 'Edit Documentation');
        ...
        btn.addEventListener('click', () => {
            btn.disabled = true;
            printLibraryControlSheet().finally(() => { btn.disabled = false; });
        });
