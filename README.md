...
| [Second Save Button - auto update omitted version](FE---Second-Save-Button.user.js) | Adds a second fixed Save button for line items |
...
| [Auto-Uploader (Photos + Scanner) - backup do NOT INSTALL](SH---Bristow-Auto-Uploader-Photos-Scanner.user.js) | Phone photo and scanner upload panel — REQUIRES Node and change IP address |
| [Orders Grid Filter Optimizer](SH---Orders-Grid-Filter-Optimizer.user.js) | Fix for Customer orders page default contains filter auto updates every 5 min (completed ON by default) |
